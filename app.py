...
# Load precomputed correlation results from external script.
# This file should be generated by your precomputation script.
try:
...
# (Optional) Helper functions that remain for scatter plot computations.
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(
            html.Ul([
                html.Li("This app maps bird abundance for selected species and energy infrastructure."),
                html.Li("Please note that these results are illustrative and have not been peer reviewed!"),
                html.Li("It uses precomputed spatial correlations between bird abundance and distance to a user-selected subset of energy features."),
                html.Li("Separate scatter plots (in km) are generated for each energy layer below the map.")
            ]),
            width=12
        )
    ], className="my-3"),
    ...
    dbc.Col(
        html.Div([
            ...
            html.Ul([
                ...
                html.Li("Additional processing details can be found at [TBD]")
            ])
...
# Callback to update the map, display precomputed correlation results, and generate individual scatter plots.
@app.callback(
...
def update_map(n_clicks, selected_bird, selected_infra):
    ...
    if selected_infra:
        for i, infra_file in enumerate(selected_infra):
            ...
            try:
                ...
                # Instead of computing correlation in real time, reference the precomputed results.
                if "abd" in bird_gdf.columns:
                    r = precomputed_correlations.get(selected_bird, {}).get(infra_file, None)
                    if r is not None:
                        correlation_results[os.path.splitext(infra_file)[0]] = r
                        
                    # Compute scatter data: x = bird "abd", y = distances in meters (later convert to km).
                    distances = compute_distances(bird_gdf, infra_gdf)
                    scatter_data.append({
                        "name": os.path.splitext(infra_file)[0],
                        "x": bird_gdf["abd"].values,
                        "y": distances,  # in meters; will convert to km in plotting
                        "color": chosen_color
                    })
            except Exception as e:
    ...
    # Append precomputed correlation analysis results to the status message.
